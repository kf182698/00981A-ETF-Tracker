...
def main():
    ...
    # ====== 新增持股（昨日=0 且 今日>門檻）=====
    new_mask = (df_merge["昨日權重%"].fillna(0) <= 0) & (df_merge["今日權重%"] >= float(NEW_WEIGHT_MIN))
    df_new = df_merge.loc[new_mask, ["股票代號","股票名稱","今日權重%"]].sort_values("今日權重%", ascending=False)
    df_new.to_csv(os.path.join(REPORT_DIR, f"new_gt_{str(NEW_WEIGHT_MIN).replace('.','p')}_{today}.csv"),
                  index=False, encoding="utf-8-sig")

    # ====== D5（近五份快照）=====
    # 找到包含 today 在內，往前找 5 份快照，不足就用就近回填
    dates_all = dates
    if today not in dates_all:
        dates_all.append(today)
        dates_all = sorted(set(dates_all))
    idx = dates_all.index(today)
    back = dates_all[max(0, idx-4):idx+1]  # 最多 5 份
    # 若不足 5 份，用最早那份回填
    while len(back) < 5 and back:
        back.insert(0, back[0])

    # 建 panel：各日的 (code -> weight)
    panel = {}
    for d in back:
        df = _read_day(d)
        if df is None:
    ...
    # 對齊到「今日有持股」集合（避免移除檔產生 0→X 的巨大訊號）
    today_codes = set(df_t["股票代號"].astype(str))
    all_codes = today_codes
    df_d5 = pd.DataFrame({
        "今日%":   w_today.reindex(all_codes).fillna(0.0),
        "昨日%":   w_yest.reindex(all_codes).fillna(0.0),
        "T-5日%":  w_t5.reindex(all_codes).fillna(0.0),
    })
    # 取名稱（用今日）
