# charts.py — English titles + legends + deterministic filenames
import os, re, glob
import pandas as pd
...
def _read_day(date_str):
    ...
    col = None
    for c in ["持股權重","持股比例","權重","占比","比重(%)","占比(%)","Weight"]:
...
def _pick_top5_for_trend(today):
    ...
    if p5:
        ...
        if "D5Δ%" in df.columns:
            ...
            sel = df.sort_values("absD5", ascending=False).head(5)["股票代號"].astype(str).tolist()
            return sel
    df_t = _read_day(today)
    if df_t is None: return None
    return df_t.sort_values("w", ascending=False).head(5)["股票代號"].astype(str).tolist()

def chart_d1_bar(today):
    updown = _read_latest(os.path.join(REPORT_DIR, f"up_down_today_{today}.csv")) \
             or _read_latest(os.path.join(REPORT_DIR, "up_down_today_*.csv"))
    if not updown: 
        print("[charts] no up_down_today report"); 
        return None
    ...
    labels = [str(r["股票代號"]) for _, r in comb.iterrows()]
    vals = comb["Δ%"].tolist()
    ...
    plt.title(f"D1 Weight Change (Top {TOP_N} Up/Down) - {today}")
...
def chart_daily_trend(today):
    ...
    mat = pd.DataFrame(panel).T.sort_index().fillna(method="ffill")
    ...
    out = os.path.join(CHART_DIR, f"daily_trend_{today}.png")
    plt.tight_layout(); plt.savefig(out, dpi=150); plt.close()
    print("[charts] saved:", out)
    return out
